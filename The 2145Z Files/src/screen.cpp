...
lv_obj_t* logoImg = lv_image_create(autoSelector);
lv_obj_t* colorInd = lv_obj_create(autoSelector);
lv_obj_t* colorOverlay = lv_image_create(autoSelector);
lv_obj_t* allianceInd = lv_obj_create(autoSelector);
lv_obj_t* allianceOverlay = lv_image_create(autoSelector);
...
lv_obj_t* autonField = lv_image_create(autoSelector);
lv_obj_t* autonRobot = lv_image_create(autonField);
...
LV_IMAGE_DECLARE(colorindOverlay);
LV_IMAGE_DECLARE(matchField);
LV_IMAGE_DECLARE(skillsField);
...
void AutonSel::selector_populate(std::vector<AutonObj> auton_list) { autons.insert(autons.end(), auton_list.begin(), auton_list.end()); }
...
int pathIter = 0;
std::vector<Coordinate> pathDisplay;

void resetViewer(bool full) {
    if(full) {
        auto preference = curMatchState;
        curMatchState = MatchState::DISABLED;
        autonPath = {};
        auton_sel.selector_callback();
        pathDisplay = injectPath(autonPath, 1);
        curMatchState = preference;
        lv_image_set_src(autonField, &(currentField == Fields::MATCH ? matchField : skillsField));
    }
    pathIter = 0;
}
        ...
        if(pathIter < pathDisplay.size() && pathDisplay.size() > 1 && playing) {
            ...
            if(pathIter < pathDisplay.size() - 1) {
                ...
                if(pathDisplay[pathIter].left == KEY)
                    ...
                    pros::delay(1000 * std::abs(get_time_point(1, velocity)));
                }
            }
...
void colorSet(Alliances color, lv_obj_t* object) {
    ...
    lv_color_t color_use = theme_accent;
...
void uiInit() {
    ...
    lv_obj_set_style_transform_rotation(autoSelector, 0, LV_PART_MAIN);
    // colorInd setup
    lv_obj_remove_flag(colorInd, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_move_to_index(colorInd, lv_obj_get_child_count(lv_obj_get_parent(colorInd)) - 1);

    // colorOverlay setup
    lv_image_set_src(colorOverlay, &colorindOverlay);
    ...
    lv_obj_set_style_image_recolor_opa(colorOverlay, 255, LV_PART_MAIN);
    lv_obj_set_style_image_recolor(colorOverlay, theme_color, LV_PART_MAIN);
    lv_obj_add_flag(colorOverlay, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_move_to_index(colorOverlay, lv_obj_get_child_count(lv_obj_get_parent(colorOverlay)) - 1);

    // allianceInd setup
    lv_obj_remove_flag(allianceInd, LV_OBJ_FLAG_SCROLLABLE);
    ...
    lv_obj_move_to_index(allianceInd, lv_obj_get_child_count(lv_obj_get_parent(allianceInd)) - 1);

    // allianceOverlay setup
    lv_image_set_src(allianceOverlay, &colorindOverlay);
    ...
    lv_obj_set_style_image_recolor_opa(allianceOverlay, 255, LV_PART_MAIN);
    lv_obj_set_style_image_recolor(allianceOverlay, theme_color, LV_PART_MAIN);
    lv_obj_add_flag(allianceOverlay, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_move_to_index(allianceOverlay, lv_obj_get_child_count(lv_obj_get_parent(allianceOverlay)) - 1);
...
static void selectAuton(lv_event_t* e) {
    ...
    for(int i = 0; i < lv_obj_get_child_count(autonTable); i++) {
        lv_obj_t* auton = lv_obj_get_child(autonTable, i);
        lv_obj_remove_state(auton, LV_STATE_CHECKED);
    }
...
static void angleCheckEvent(lv_event_t* e) {
    angleViewer = lv_msgbox_create(NULL);
    lv_msgbox_add_title(angleViewer, "check alignment");
    lv_msgbox_add_text(angleViewer, "0°");
    lv_obj_t *closeBtn = lv_msgbox_add_close_button(angleViewer);
    angleText = lv_msgbox_get_content(angleViewer);
    aligning = true;

    lv_obj_add_event_cb(closeBtn, AngleCheckCloseEvent, LV_EVENT_PRESSED, NULL);
    lv_obj_add_style(closeBtn, &pushback, LV_PART_MAIN);
    ...
    lv_obj_set_style_text_font(closeBtn, &lv_font_montserrat_24, LV_PART_MAIN);
...
static void toggleConsoleEvent(lv_event_t* e) {
    ...
    if (showConsole) {
        lv_obj_remove_flag(autonTable, LV_OBJ_FLAG_HIDDEN);
        ...
        lv_obj_remove_flag(console_container, LV_OBJ_FLAG_HIDDEN);
...
void autoSelectorInit() {
    ...
    lv_obj_set_style_transform_rotation(autonTable, 0, LV_PART_MAIN);
    ...
    for(int i = 0; i < auton_sel.autons.size(); i++) {
        lv_obj_t* new_auto = lv_list_add_button(autonTable, NULL, (" " + auton_sel.autons[i].name).c_str());
    ...
    lv_image_set_src(autonField, &matchField);
